...
namespace Rabobank.Compliancy.Core.Approvals.Tests;
...
public class FixtureClient : IAzdoRestClient
{
    ...
    public Task<IEnumerable<TResponse>> GetAsync<TResponse>(IEnumerableRequest<TResponse> request, params HttpStatusCode[] allowedHttpStatusCodes) =>
        Task.FromResult(_fixture.CreateMany<TResponse>());

    public Task<TResponse> PostAsync<TInput, TResponse>(IAzdoRequest<TInput, TResponse> request, TInput body, string organization = null, bool retry = false)
        where TResponse : new() =>
    ...
    public Task<TResponse> PatchAsync<TInput, TResponse>(IAzdoRequest<TInput, TResponse> request, TInput body, string organization = null)
        where TResponse : new()
    {
        throw new NotImplementedException();
    }

    public Task<TResponse> PutAsync<TInput, TResponse>(IAzdoRequest<TInput, TResponse> request, TInput body, string organization = null)
        where TResponse : new()
    {
        throw new NotImplementedException();
    }
