...
public class PipelineBreakerService : IPipelineBreakerService
{
    ...
    {        
        ...
        return result ?? new PipelineBreakerRegistrationReport { Result = Rabobank.Compliancy.Domain.Enums.PipelineBreakerResult.None };
    }
    ...
    public async Task<PipelineBreakerReport?> GetPreviousComplianceResultAsync(PipelineRunInfo runInfo)
    {
        ...
        return result ?? new PipelineBreakerReport { Result = Rabobank.Compliancy.Domain.Enums.PipelineBreakerResult.None };
    }
    ...
    {
        var project = await _azdoClient.GetAsync(Rabobank.Compliancy.Infra.AzdoClient.Requests.Project.ProjectById(
        ...
        {
            throw new InvalidOperationException(Configurations.ErrorMessages.ReleaseNotAvailableErrorMessage(runInfo.RunId));
        }
    ...
    private async Task<PipelineRunInfo> EnrichWithBuildInfoAsync(PipelineRunInfo runInfo)
    {
        ...
        if (build == null)
        {
            throw new InvalidOperationException(Configurations.ErrorMessages.BuildNotAvailableErrorMessage(runInfo.RunId));
        }
    ...
    private static IList<StageReport> GetPipelineStages(string yamlUsedInRun)
    {
        ...
        // convert the following code using system.text.json       
        ...
        JsonDocument doc = JsonDocument.Parse(yamlUsedInRun);
        JsonElement root = doc.RootElement;
        JsonElement stagesElement = root.GetProperty("stages");
        foreach (var stageElement in stagesElement.EnumerateArray())
        {
            var stageReport = new StageReport();
            stageReport.Id = stageElement.GetProperty("stage").ToString();
            stageReport.Name = stageElement.GetProperty("stage").ToString();
        ...
        ICollection<StageReport> stages)
    {
        ...
        /* TODO fix this later as it has dependency to function libs unnecessarily
        ...
        return ItemTypes.YamlPipelineWithStages;
    }
