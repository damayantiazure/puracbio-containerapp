...
public static class ClassicReleasePipelineHelper
{
    ...
    public static void AddConditionToEnvironments(JToken definition, string artifact, string stageId)
    {
        ...
        foreach (var environment in environments)
        {
            ...
            if (conditions.Any(c =>
                    (string)c["name"] == artifact && 
                    c["value"].Value<string>().Contains("master", StringComparison.InvariantCulture)))
            {
                continue;
            }
            ...
            conditions.Add(JToken.FromObject(new
            {
                ...
                value = "{\"sourceBranch\":\"master\",\"tags\":[],\"useBuildDefinitionBranch\"" +
                        ":false,\"createReleaseOnBuildTagging\":false}"
            }));
