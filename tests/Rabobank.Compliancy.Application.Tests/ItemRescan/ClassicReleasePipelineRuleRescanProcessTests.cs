...
public class ClassicReleasePipelineRuleRescanProcessTests : PipelineRuleRescanProcessTests
{
    [Fact]
    public async Task GetPipeline_CallsGetPipeline_WithCorrectParameters()
    {
        ...
        _pipelineService.Setup(p => p.GetPipelineAsync(parentProject, request.PipelineId, Domain.Enums.PipelineProcessType.DesignerRelease, It.IsAny<CancellationToken>())).ReturnsAsync(pipeline).Verifiable();

        await new ClassicReleasePipelineRuleRescanProcess(_ruleProcessor.Object, _projectService.Object, _reportService.Object, _pipelineService.Object)
