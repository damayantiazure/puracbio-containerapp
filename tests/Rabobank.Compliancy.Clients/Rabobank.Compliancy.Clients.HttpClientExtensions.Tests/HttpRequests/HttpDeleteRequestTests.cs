using Rabobank.Compliancy.Clients.HttpClientExtensions.Tests.HttpRequests.TestRequestImplementations;
using System.Net.Http.Headers;

namespace Rabobank.Compliancy.Clients.HttpClientExtensions.Tests.HttpRequests;

public class HttpDeleteRequestTests : HttpRequestTestBase
{
    [Fact]
    public async Task ExecuteHttpRequest_CallsHandleDeleteCall_WithCorrectUri()
    {
        // Arrange
        var sut = new HttpDeleteRequestTestImplementation(_httpCallHandler.Object);
        var calculatedUriPath = sut.GetProtectedUrl() + sut.GetProtectedQueryStringAsString();

        // Act
        await sut.ExecuteAsync();

        // Assert
        _httpCallHandler.Verify(h =>
            h.HandleDeleteCallAsync<object>(new Uri(calculatedUriPath, UriKind.Relative),
                It.IsAny<AuthenticationHeaderValue>(), It.IsAny<CancellationToken>()));
    }
}